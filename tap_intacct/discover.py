...
def discover_streams(config, tables_names):
    streams = []

    exported_tables = tables_names #s3.get_exported_tables(config['bucket'], config['company_id'], path=config.get('path'))
...
def discover_streams_for_discover_mode(config):
    streams = []
